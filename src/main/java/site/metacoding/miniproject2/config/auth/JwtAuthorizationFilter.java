...
public class JwtAuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        logger.debug("디버그 jwtToken:" + jwtToken.trim());
        try {
            ...
            logger.debug("디버그 : userId : " + userId);
            AuthRespDto authRespDto = new AuthRespDto(id, userId, companyId);
            SessionUsers principal = new SessionUsers(authRespDto);
            HttpSession session = req.getSession();
            logger.debug("디버그 세션: " + req.getSession());
            session.setAttribute("principal", principal);
            logger.debug("디버그 : id : " + principal.getId());
            logger.debug("디버그 : userId : " + principal.getUserId());
            logger.debug("디버그 : companyId : " + principal.getCompanyId());
